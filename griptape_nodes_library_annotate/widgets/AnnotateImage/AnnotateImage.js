...
import {
  ...
  DEFAULT_ARROW_WIDTH, MIN_ARROW_WIDTH, MAX_ARROW_WIDTH, DEFAULT_ARROW_SCALE,
...
export default function AnnotateImageSimple(container, props) {
  ...
  async function _doRender(gen) {
    ...
    if (currentArrow) {
      ...
      drawArrowLine(
        ...
        ts.color || DEFAULT_COLOR, ts.width || DEFAULT_ARROW_WIDTH, ts.arrow_scale ?? DEFAULT_ARROW_SCALE,
        null, null, null, null,
        ts.start_arrow_shape ?? (ts.has_start_arrow ? "triangle" : "none"),
        ts.end_arrow_shape   ?? (ts.has_end_arrow !== false ? "triangle" : "none"),
        ts.taper ?? false, ts.taperMin ?? 0
      );
    }
  ...
  function startTextEdit(ann) {
    ...
    textInput.addEventListener("blur", (e) => {
      ...
      if (toEl && headerBar.contains(toEl)) {
    ...
    if (activeTool === "paint" && currentStroke && currentStroke.points.length >= 1) {
      ...
      if (Math.hypot(arr.x2 - arr.x1, arr.y2 - arr.y1) > 5) {
        ...
        const ann = {
          ...
          color: ts.color,
          width: ts.width,
          arrow_scale: ts.arrow_scale ?? DEFAULT_ARROW_SCALE,
  ...
  const _hudMod = createHud(objectActionsEl, {
    addTooltip: _addTooltip,
